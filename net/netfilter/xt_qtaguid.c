...
/*===========================================================================*/
/* The following extract_icmp_fields() and qtaguid_find_sk() are
 * mostly a copy of xt_socket.c (extract_icmp_fields, socket_match()).
 * But extract_icmp_fields() is static in that file, and thus can't be directly
 * used.
 * And socket_match() would need to be cut up and return values changed.
 * Which is hard to sell upstream when xt_qtaguid is not pushed at the same
 * time. So for now we keep our own version.
