...
#ifdef CONFIG_FREQ_STATS_ACCOUNTING

#ifdef CONFIG_BEGIN_ACCOUNTING_FROM_BOOT
static int freq_stats_on = 1;
#else
static int freq_stats_on;
#endif /*CONFIG_BEGIN_ACCOUNTING_FROM_BOOT*/
				...
				bool indent, int level)
{
	...
	if (!clk->current_freq_stats && clk->start_time)
		seq_printf(m, "%40llu",
			clk->zero_freq_time + get_jiffies_64()
			- clk->start_time);
	else
		seq_printf(m, "%40llu", clk->zero_freq_time);
	...
	for (pos = rb_first(&clk->freq_stats_table); pos; pos = rb_next(pos)) {
		...
		if (cur->rate == clk->rate && clk->start_time)
			seq_printf(m, "%40llu",
				cur->time_spent + get_jiffies_64()
				- clk->start_time);
		else
			seq_printf(m, "%40llu", cur->time_spent);
...
#endif /*CONFIG_FREQ_STATS_ACCOUNTING*/
...
static void clk_traverse_subtree(struct clk *clk, int freq_stats_on)
{
	...
	if (freq_stats_on) {
		for (node = rb_first(&clk->freq_stats_table);
			...
			rb_entry(node, struct freq_stats, node)->time_spent = 0;
