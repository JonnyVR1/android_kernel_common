...
#define BREAK_COMPILE_NE(_enum, _array)			\
{ switch (0) {case 0:case _enum == ARRAY_SIZE(_array):; } }
...
int dual_role_set_property(struct dual_role_phy_instance *dual_role,
			   enum dual_role_property prop,
			   const union dual_role_propval *val)
{
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	union dual_role_propval value;

	if (off == DUAL_ROLE_PROP_SUPPORTED_MODES) {
		value.intval = dual_role->desc->supported_modes;
	} else {
	...
	if (off == DUAL_ROLE_PROP_SUPPORTED_MODES)
		if (value.intval < DUAL_ROLE_PROP_SUPPORTED_MODES_TOTAL)
			return snprintf(buf, PAGE_SIZE, "%s\n",
					supported_modes_text[value.intval]);
		else
			return -EIO;
	else if (off == DUAL_ROLE_PROP_MODE)
		if (value.intval < DUAL_ROLE_PROP_MODE_TOTAL)
			return snprintf(buf, PAGE_SIZE, "%s\n",
					mode_text[value.intval]);
		else
			return -EIO;
	else if (off == DUAL_ROLE_PROP_PR)
		if (value.intval < DUAL_ROLE_PROP_PR_TOTAL)
			return snprintf(buf, PAGE_SIZE, "%s\n",
					pr_text[value.intval]);
		else
			return -EIO;
	else if (off == DUAL_ROLE_PROP_DR)
		if (value.intval < DUAL_ROLE_PROP_DR_TOTAL)
			return snprintf(buf, PAGE_SIZE, "%s\n",
					dr_text[value.intval]);
		else
			return -EIO;
	else if (off == DUAL_ROLE_PROP_VCONN_SUPPLY)
		if (value.intval < DUAL_ROLE_PROP_VCONN_SUPPLY_TOTAL)
			return snprintf(buf, PAGE_SIZE, "%s\n",
					vconn_supply_text[value.intval]);
		else
			return -EIO;
	else
		return -EIO;
					...
					const char *buf, size_t count)
{
	...
	case DUAL_ROLE_PROP_VCONN_SUPPLY:
		total = DUAL_ROLE_PROP_VCONN_SUPPLY_TOTAL;
		text_array = vconn_supply_text;
		break;
	}
...
{
	BREAK_COMPILE_NE(DUAL_ROLE_PROP_SUPPORTED_MODES_TOTAL,
			 supported_modes_text);
	BREAK_COMPILE_NE(DUAL_ROLE_PROP_MODE_TOTAL, mode_text);
	BREAK_COMPILE_NE(DUAL_ROLE_PROP_PR_TOTAL, pr_text);
	BREAK_COMPILE_NE(DUAL_ROLE_PROP_DR_TOTAL, dr_text);
	BREAK_COMPILE_NE(DUAL_ROLE_PROP_VCONN_SUPPLY_TOTAL, vconn_supply_text);
