...
struct usb_function_instance *alloc_inst(bool mtp_config)
{
	struct mtp_instance *fi_mtp;
	int ret;
	...
	if (mtp_config) {
		...
		if (ret) {
			printk(KERN_ERR"Error setting MTP\n");
			return ret;
		}
	} else {
		fi_mtp->dev = _mtp_dev;
...
struct usb_function *function_alloc(struct usb_function_instance *fi, bool mtp_config)
{
...
EXPORT_SYMBOL_GPL(function_alloc);
