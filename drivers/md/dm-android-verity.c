...
static inline u64 fec_div_round_up(u64 x, u64 y)
{
	...
	return div64_u64_rem(x, y, &reminder) +
		(reminder > 0 ? 1 : 0);
}
...
{
	ecc->blocks = fec_div_round_up(header->inp_size, FEC_BLOCK_SIZE);
	ecc->roots = header->roots;
	ecc->start = header->inp_size;
}
...
static int android_verity_ctr(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	*verity_table_args[VERITY_TABLE_ARGS + 1 + VERITY_TABLE_OPT_FEC_ARGS];
	...
	if (sscanf(verity_table_args[5], "%llu%c", &data_sectors, &dummy)
							!= 1) {
		DMERR("Verity table not in the expected format\n");
		handle_error();
		err = -EINVAL;
