...
int create_iface_stat(struct in_device *in_dev) {
	...
	if ((entry = get_iface_stat(dev->name)) != NULL) {
		...
		if (ipv4_is_loopback(ipaddr)) {
			entry->active = false;
			/*
			   printk(KERN_INFO "iface_stat: Disabling monitor for
			   		loopback device %s\n", ifname);
			 */
		} else {
			entry->active = true;
			/*
			   printk(KERN_INFO "iface_stat: Re-enabling monitor for
		...
		/*
		   printk(KERN_DEBUG "iface_stat: Ignoring monitor for
...
int iface_stat_update(struct net_device *dev) {
	...
	if (entry->active) { /* FIXME: Support for more than 4GB */
		...
		entry->active = false;
		/*
		   printk(KERN_DEBUG "iface_stat: Updating stats for
		   		dev %s which went down\n", dev->name);
		 */
	} else {
		/*
		   printk(KERN_DEBUG "iface_stat: Did not update stats for
		   		dev %s which went down\n", dev->name);
		 */
	}
