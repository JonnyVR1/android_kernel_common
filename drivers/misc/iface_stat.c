...
void create_iface_stat(const struct in_device *in_dev)
{
	...
	create_proc_read_entry("tx_bytes", S_IRUGO, proc_entry,
			read_proc_entry, (void *) &new_iface->tx_bytes);

	create_proc_read_entry("rx_bytes", S_IRUGO, proc_entry,
			read_proc_entry, (void *) &new_iface->rx_bytes);

	create_proc_read_entry("tx_packets", S_IRUGO, proc_entry,
			read_proc_entry, (void *) &new_iface->tx_packets);

	create_proc_read_entry("rx_packets", S_IRUGO, proc_entry,
			read_proc_entry, (void *) &new_iface->rx_packets);

	create_proc_read_entry("active", S_IRUGO, proc_entry,
			read_proc_bool_entry, (void *) &new_iface->active);
...
static void iface_worker_func(struct work_struct *work)
{
	struct create_work_struct_t *create_work
		= (struct create_work_struct_t *)work;
		...
		enum tx_rx direction, enum tcp_udp transport)
{
	...
	if (!uid_entry_exists) {
		 ...
		 */
		create_work = kmalloc(sizeof(struct create_work_struct_t),
					GFP_ATOMIC);
		if (create_work == NULL) {
...
void if_uid_stat_update_tx_tcp(const char *devname, uid_t uid, int bytes)
{
	if (0 != if_uid_stat_update(devname, uid, bytes, TX, TCP))
		pr_debug("iface_stat: Error updating tx_tcp stats for "
				"uid=%d.\n", uid);
}
...
void if_uid_stat_update_rx_tcp(const char *devname, uid_t uid, int bytes)
{
	if (0 != if_uid_stat_update(devname, uid, bytes, RX, TCP))
		pr_debug("iface_stat: Error updating rx_tcp stats for "
				"uid=%d.\n", uid);
}
...
void if_uid_stat_update_tx_udp(const char *devname, uid_t uid, int bytes)
{
	if (0 != if_uid_stat_update(devname, uid, bytes, TX, UDP))
		pr_debug("iface_stat: Error updating tx stats for "
			       "non-TCP traffic for uid=%d.\n", uid);
}

void if_uid_stat_update_rx_udp(const char *devname, uid_t uid, int bytes)
{
	if (0 != if_uid_stat_update(devname, uid, bytes, RX, UDP))
		pr_debug("iface_stat: Error updating rx_udp stats for "
				"uid=%d.\n", uid);
}

void debug_print_skbuff_contents(const struct sk_buff *skb,
				const char *ifname, int uid,
				int success, int tx)
{
	...
	case IPPROTO_TCP:
		...
		pr_debug("iface_stat: uid %d, dev %s, %s %s "
				"proto TCP\t %pI4 -> %pI4 %d bytes\n",
				uid,
				ifname,
				tx == 1 ? "TX" : "RX",
