...
void create_iface_stat(const struct in_device *in_dev) {
	...
	if ((entry = get_iface_stat(dev->name)) != NULL) {
		...
		if (ipv4_is_loopback(ipaddr)) {
			entry->active = false;
			pr_debug("iface_stat: Disabling monitor for loopback device %s\n", ifname);
		} else {
			entry->active = true;
			pr_debug("iface_stat: Re-enabling monitor for device %s with ip %pI4\n",
					ifname, &ipaddr);
		}
		return;
	} else if (ipv4_is_loopback(ipaddr)) {
		pr_debug("iface_stat: Ignoring monitor for loopback device %s with ip %pI4\n",
				ifname, &ipaddr);
		return;
	}
	...
	if ((new_iface->iface_name = kmalloc((strlen(ifname)+1)*sizeof(char), GFP_KERNEL)) == NULL) {
		kfree(new_iface);
		return;
	}
	...
	create_proc_read_entry("active", S_IRUGO, proc_entry, read_proc_bool_entry,
			(void *) &new_iface->active);
...
static void create_if_uid_stat(struct iface_stat *iface_entry, uid_t uid, enum tx_rx direction,
	...
	switch (transport) {
		case TCP:
			switch (direction) {
				case TX:
					new_uid->tx_bytes_tcp += bytes;
					new_uid->tx_packets_tcp += 1;
					break;
				case RX:
				default:
					new_uid->rx_bytes_tcp += bytes;
					new_uid->rx_packets_tcp += 1;
					break;
			}
			break;
		case UDP:
		default:
			switch (direction) {
				case TX:
					new_uid->tx_bytes_udp += bytes;
					new_uid->tx_packets_udp += 1;
					break;
				case RX:
				default:
					new_uid->rx_bytes_udp += bytes;
					new_uid->rx_packets_udp += 1;
					break;
			}
	}
...
{
	struct create_work_struct_t *create_work = (struct create_work_struct_t *)work;
	create_if_uid_stat(create_work->iface_entry, create_work->uid, create_work->direction,
				create_work->transport, create_work->bytes);
	kfree((void *)work);
}
		...
		enum tx_rx direction, enum tcp_udp transport)
{
	struct uid_stat *uid_entry;
	struct iface_stat *iface_entry;
	struct create_work_struct_t *create_work;
	unsigned long flags;
	...
	if (!uid_entry_exists) {
		 ...
		 */
		create_work = (struct create_work_struct_t *) kmalloc(sizeof(struct create_work_struct_t), GFP_ATOMIC);
		if (create_work == NULL) {
...
{
	iface_parent = proc_mkdir("iface_stat", NULL);
	if (!iface_parent) {
