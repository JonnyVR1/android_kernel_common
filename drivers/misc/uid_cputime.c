...
static int uid_stat_show(struct seq_file *m, void *v)
{
	...
	for_each_process(task) {
		uid_entry = find_or_register_uid(from_kuid(current_user_ns(), task_uid(task)));
		if (!uid_entry) {
			read_unlock(&tasklist_lock);
			mutex_unlock(&uid_lock);
			pr_err("%s: failed to find the uid_entry for uid %d\n",
						__func__, from_kuid(current_user_ns(), task_uid(task)));
			return -ENOMEM;
		}
			...
			unsigned long cmd, void *v)
{
	...
	mutex_lock(&uid_lock);
	uid = from_kuid(current_user_ns(), task_uid(task));
