...
static int get_bw_buckets(struct device *dev)
{
	...
	ret = of_property_read_u32(node, NUM_SOURCES, &num_sources);
	if (ret < 0) {
		pr_err("Unable to read num sources from device tree.\n");
		return ret;
	}
...
static int __init memory_state_time_init(void)
{
	...
	if (error) {
		destroy_workqueue(memory_wq);
		kobject_put(memory_kobj);
		sysfs_remove_group(memory_kobj, &memory_attr_group);
