			       ...
			       int count, int *peof, void *dat)
{
	...
	page_offset = (file_offset + offset) % ctx->mtd->writesize;
        ...
        printk("%s: fl %u fo %u ro %u cnt %u, pn %u po %u\n",
               __func__, file_length, file_offset, offset, count,
               page_no, page_offset);
...
static int apanic_writeflashpage(struct mtd_info *mtd, loff_t to, const u_char *buf)
{
	int rc;
	size_t wlen;
