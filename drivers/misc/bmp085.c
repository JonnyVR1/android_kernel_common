...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void bmp085_early_suspend(struct early_suspend *handler);
static void bmp085_late_resume(struct early_suspend *handler);
#endif
...
static void bmp085_device_power_off(struct bmp085_data *barom)
{
	if (barom->pdata->power_off)
		barom->pdata->power_off();

	return;
}

static int bmp085_device_power_on(struct bmp085_data *barom)
{
	int err;

	if (barom->pdata->power_on) {
		err = barom->pdata->power_on();
		if (err < 0)
			return err;
	}
...
static int bmp085_enable(struct bmp085_data *barom)
{
	...
	if (!atomic_cmpxchg(&barom->enabled, 0, 1)) {
		...
		err = bmp085_device_power_on(barom);
		if (err < 0) {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	barom->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	barom->early_suspend.suspend = bmp085_early_suspend;
	barom->early_suspend.resume = bmp085_late_resume;
	register_early_suspend(&barom->early_suspend);
#endif
...
static void bmp085_early_suspend(struct early_suspend *handler)
{
	...
	barom = container_of(handler, struct bmp085_data, early_suspend);
	bmp085_suspend(barom->client, PMSG_SUSPEND);
}
