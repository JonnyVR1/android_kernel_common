/* drivers/i2c/akm8975.c - akm8975 compass driver
 *
...
static int AKECS_TransRBuff(char *rbuf, int size)
{
	...
	if (!atomic_read(&data_ready)) {
		if (!atomic_read(&suspend_flag)) {
			pr_err("AKECS_TransRBUFF: Data not ready\n");
			failure_count++;
			if (failure_count >= MAX_FAILURE_COUNT) {
	pr_err("AKECS_TransRBUFF: successive %d failure.\n",
				 failure_count);
...
static int
akm_aot_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	      unsigned long arg)
{
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	if (!akm->input_dev) {
		err = -ENOMEM;
		pr_err("akm8975_probe: Failed to allocate input device\n");
