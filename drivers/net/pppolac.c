...
static int pppolac_recv(struct sock *sk_udp, struct sk_buff *skb)
{
	...
	if (skb->data[sizeof(struct udphdr)] & 0x80) {
		return 1;
	}

	/* Now the packet is ours. Skip UDP header. */
	__skb_pull(skb, sizeof(struct udphdr));
	flag = skb->data[0] & 0x4A;
	ptr = &skb->data[2];

	/* Check the protocol version. */
	if ((skb->data[1] & 0x0F) != 0x02) {
		goto drop;
	}
	...
	if (!skb_pull(skb, 6 + (flag >> 5) + (flag >> 1 & 4) + (flag & 2))) {
		goto drop;
	}

	/* Skip the offset padding if it is present. */
	if (flag & 0x02 && !skb_pull(skb, skb->data[-2] << 8 | skb->data[-1])) {
		goto drop;
	}
	...
	int addrlen, int flags)
{
	...
	struct socket *sock_udp;
	struct sock *sk_udp;
	int error;

	if (addr->sa_protocol != PX_PROTO_OLAC ||
	...
	/* Remove this when IPv6 supports UDP encapsulation. */
	...
	memcpy(&po->proto.lac.local, &addr->local, 4);
	memcpy(&po->proto.lac.remote, &addr->remote, 4);
