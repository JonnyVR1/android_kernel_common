...
static void *ion_page_pool_alloc_page(struct ion_page_pool *pool)
{
	struct page *page = alloc_pages(pool->gfp_mask, pool->order);

	if (!page)
		return NULL;
	/* this is only being used to flush the page for dma, no mappings
	   are created so it is unnecessary to call dma_unmap_page later
	   this violates the dma api but is the best method for flushing
	   a page for device at this time */
	dma_map_page(NULL, page, 0, (1 << pool->order) * PAGE_SIZE,
		     DMA_BIDIRECTIONAL);
	return page;
}

static void ion_page_pool_free_page(struct ion_page_pool *pool, struct page *page)
{
...
void *ion_page_pool_alloc(struct ion_page_pool *pool)
{
	...
	if (pool->count)
		...
		page = ion_page_pool_alloc_page(pool);
...
void ion_page_pool_free(struct ion_page_pool *pool, struct page* page)
{
	...
	if (ret)
		ion_page_pool_free_page(pool, page);
	mutex_unlock(&pool->mutex);
}
