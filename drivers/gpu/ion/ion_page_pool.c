...
static struct plist_head pools;
				 ...
				 struct shrink_control *sc)
{
	...
	pr_info("%s: shrunk page_pool by %d pages\n", __func__, nr_freed);
...
static int __init ion_page_pool_init(void)
{
	...
	shrinker.seeks = DEFAULT_SEEKS * 16;
	shrinker.batch = 0;
...
static int __init ion_page_pool_setup(void)
{
	plist_head_init(&pools);
	return 0;
}

pure_initcall(ion_page_pool_setup);
