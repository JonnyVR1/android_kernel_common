...
static size_t ion_heap_free_list_size(struct ion_heap *heap)
{
	return atomic64_read(&heap->free_list_size);
}

static void ion_heap_free_list_size_sub(struct ion_heap *heap, size_t val)
{
	atomic64_sub(val, &heap->free_list_size);
}

static void ion_heap_free_list_size_add(struct ion_heap *heap, size_t val)
{
	atomic64_add(val, &heap->free_list_size);
}
...
size_t ion_heap_drain_freelist(struct ion_heap *heap)
{
	...
	if (ion_heap_free_list_is_empty(heap))
		return 0;
	rt_mutex_lock(&heap->lock);
	freed_size = ion_heap_free_list_size(heap);
