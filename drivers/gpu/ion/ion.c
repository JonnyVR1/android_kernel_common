 ...
 * view.  The mutex stored here is used to protect both handles tree
			     ...
			     size_t align, unsigned int flags)
{
	 ...
	 * order.  If the heap type is a mappable by the mapper, allocate
	 * from it.  Repeat until allocate has succeeded or all heaps have been
	 * tried
	 */
	mutex_lock(&dev->lock);
...
static void ion_vma_open(struct vm_area_struct *vma)
{
	...
	pr_debug("%s: %d client_cnt %d handle_cnt %d handle_map_cnt %d "
	       "alloc_cnt %d\n", __func__, __LINE__,
	       atomic_read(&client->ref.refcount),
