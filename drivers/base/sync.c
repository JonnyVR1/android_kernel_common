	...
	const char *name)
{
	...
	list_add_tail(&obj->sync_obj_list, &sync_obj_list_head);
	spin_unlock_irqrestore(&sync_obj_list_lock, flags);

	return obj;
}

static void sync_obj_free(struct sync_obj *obj)
{
	unsigned long flags;
...
void sync_obj_destroy(struct sync_obj *obj)
{
	...
	if (list_empty(&obj->child_list_head))
		sync_obj_free(obj);
	else
		sync_obj_signal(obj);
}

static void sync_obj_add_pt(struct sync_obj *obj, struct sync_pt *pt)
{
	unsigned long flags;
...
static void sync_obj_remove_pt(struct sync_pt *pt)
{
	...
	if (obj->destroyed && list_empty(&obj->child_list_head))
		sync_obj_free(obj);
}
...
static int _sync_pt_test(struct sync_pt *pt)
{
	...
	if (!pt->status)
		pt->status = pt->parent->ops->test(pt);
...
static struct file_operations sync_fence_fops = {
...
int sync_fence_wait(struct sync_fence *fence, long timeout)
{
	...
	if (timeout) {
		err = wait_event_interruptible_timeout(fence->wq, fence->status != 0,
						       timeout);
	} else {
...
static long sync_fence_ioctl_wait(struct sync_fence *fence, unsigned long arg)
{
	...
	return sync_fence_wait(fence, msecs_to_jiffies(value));
}
...
static long sync_fence_ioctl_fence_info(struct sync_fence *fence, unsigned long arg)
{
...
static void sync_print_obj(struct seq_file *s, struct sync_obj *obj)
{
	struct list_head *pos;
	unsigned long flags;
...
static int sync_debugfs_show(struct seq_file *s, void *unused)
{
	...
	spin_lock_irqsave(&sync_obj_list_lock, flags);
	list_for_each(pos, &sync_obj_list_head) {
		struct sync_obj *obj =
			container_of(pos, struct sync_obj, sync_obj_list);
