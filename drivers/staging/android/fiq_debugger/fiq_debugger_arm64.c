...
static char *mode_name(struct pt_regs *regs)
{
...
static int report_trace(struct stackframe *frame, void *d)
{
	...
	if (sts->depth) {
		fiq_debugger_printf(sts->state, "  pc:    %p (%pF)\n",
				frame->pc, frame->pc);
		fiq_debugger_printf(sts->state, "     sp: %p  fp %p\n",
				frame->sp, frame->fp);
		sts->depth--;
		return 0;
	}
...
void fiq_debugger_dump_stacktrace(struct fiq_debugger_state *state,
		struct pt_regs *regs, unsigned int depth, void *ssp)
{
	...
	*current_thread_info() = *real_thread_info;
	...
	if (!user_mode(regs)) {
		...
		frame.sp = regs->sp;
		frame.pc = regs->pc;
		fiq_debugger_printf(state, "  pc:    %p (%pF)\n",
				regs->pc, regs->pc);
		fiq_debugger_printf(state, "     lr: %p (%pF)\n",
				regs->regs[30], regs->regs[30]);
		fiq_debugger_printf(state, "     sp: %p  fp %p\n",
				regs->sp, regs->regs[29]);
