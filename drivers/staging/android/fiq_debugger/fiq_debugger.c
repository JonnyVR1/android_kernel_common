...
static bool fiq_uart_overlay_apply = false;
...
int fiq_debugger_uart_overlay_apply(void)
{
	...
	if (!fiq_uart_overlay_apply) {
		pr_err(FIQ_DEBUGGER_DEV " is not set as console\n");
		return -EPERM;
	}
...
static int __init fiq_debugger_console_setup(char *str)
{
	if (strncmp(str, FIQ_DEBUGGER_DEV, strlen(FIQ_DEBUGGER_DEV)) == 0)
		fiq_uart_overlay_apply = true;
	return 1;
}
__setup("androidboot.console=", fiq_debugger_console_setup);
#endif

static int __init fiq_debugger_init(void)
{
#if defined(CONFIG_FIQ_DEBUGGER_UART_OVERLAY)
	int ret = fiq_debugger_uart_overlay_apply();
