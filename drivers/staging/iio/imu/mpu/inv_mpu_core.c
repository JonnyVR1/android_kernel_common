...
/**
 * inv_dmp_flick_show() -  calling this function will show flick
 */
	...
	struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_gyro_state_s *st = iio_priv(indio_dev);
	wait_event_interruptible(mpuirq_wait, st->flick_ready);
	st->flick_ready = 0;
	return sprintf(buf, "1\n");
}
/**
	...
	struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_gyro_state_s *st = iio_priv(indio_dev);
	wait_event_interruptible(mpuirq_wait, st->orient_ready);
	st->orient_ready = 0;
	return sprintf(buf, "%d\n", st->orient_data);
}
/**
	...
	struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_gyro_state_s *st = iio_priv(indio_dev);
	wait_event_interruptible(mpuirq_wait, st->tap_ready);
	st->tap_ready = 0;
	return sprintf(buf, "%d\n", st->tap_data);
}
/**
