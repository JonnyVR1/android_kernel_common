...
static int put_scan_to_buf(struct iio_dev *indio_dev, unsigned char *d,
	...
	struct iio_buffer *ring = indio_dev->buffer;
	int st;
	st = iio_scan_mask_query(indio_dev, ring, scan_index);
	if (st) {
		d[d_ind]     = (s[0] & 0xff);
		d[d_ind + 1] = ((s[0]>>8) & 0xff);
		d_ind += 2;
	}
	st = iio_scan_mask_query(indio_dev, ring, scan_index+1);
	if (st) {
		d[d_ind]     = (s[1] & 0xff);
		d[d_ind + 1] = ((s[1]>>8) & 0xff);
		d_ind += 2;
	}
	st = iio_scan_mask_query(indio_dev, ring, scan_index+2);
	if (st) {
		d[d_ind]     = (s[2] & 0xff);
		d[d_ind + 1] = ((s[2]>>8) & 0xff);
		d_ind += 2;
	}
	return d_ind;
}
...
irqreturn_t inv_read_fifo_mpu3050(int irq, void *p)
{
	...
	*  because interrupt is now connected with enable */
	...
	while ((bytes_per_datum != 0) && (fifo_count >= byte_read)) {
		...
		if (result)
			goto flush_fifo;
		inv_report_data_3050(indio_dev, timestamp, st->fifo_counter,
					data);
		fifo_count -= byte_read;
		if (st->fifo_counter == 0) {
			st->fifo_counter = 1;
					...
					unsigned char *data, s64 t)
{
	...
	struct iio_buffer *ring = indio_dev->buffer;
	short g[3], a[3], c[3];
	int result, bytes_per_datum, i, ind, d_ind, scan_count;
	s64 buf[8];
	unsigned char d[8];
	unsigned char *tmp;
	struct inv_chip_config_s *conf;
	...
	if (conf->dmp_on) {
		...
		if (data[ind + 1] & 1) {
			st->tap_data = data[ind+3];
			st->tap_ready = 1;
			inv_wake_up();
		}
		/* report orientation information */
		if (data[ind + 1] & 2) {
			st->orient_data = data[ind+2];
			st->orient_ready = 1;
			inv_wake_up();
		}
	}
	...
	if (conf->accl_fifo_enable | conf->dmp_on)
		d_ind = put_scan_to_buf(indio_dev, tmp, a,
				INV_MPU_SCAN_ACCL_X, d_ind);
	if (conf->compass_fifo_enable | conf->dmp_on)
