...
retry:
	...
	/*
	 * There is a small chance that both cpus arrived at the waiting loop
	 * at the exact same time, which can cause one cpu to poke the another
	 * cpu, but the second cpu never handles it.  Abort and retry all cpus.
	 * The coupled_cpuidle_poked mask ensure that each cpu sees at least
	 * one poke, which ensures that the retry won't hit the same problem.
	 */
