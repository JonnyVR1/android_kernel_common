...
struct ub_device {
	...
	struct mutex ctl_io_lock; /* should be taken in control read and control write */
...
struct workqueue_struct *ub_wq;

static struct ub_request *ub_request_alloc(const struct ub_request_type *type)
{
	struct ub_request *req;

	req = kmalloc(sizeof(struct ub_request), GFP_ATOMIC);
	if (req)
		req->type = type;
	return req;
}
...
static void ub_enqueue(struct ub_device *dev, struct ub_request *req,
		       int assign_seq)
{
...
static void ub_await(struct ub_device *dev, struct ub_request *req)
{
...
static struct ub_request *ub_find_out(struct ub_device *dev, u32 seq)
{
	struct list_head *pos;
	struct ub_request *req;
...
static void __ub_retire(struct ub_device *dev, struct ub_request *req)
{
	list_del(&req->list);
	dev->nr_out--;
}

static void ub_retire(struct ub_device *dev, struct ub_request *req)
{
	mutex_lock(&dev->out_lock);
	__ub_retire(dev, req);
	mutex_unlock(&dev->out_lock);
...
static struct ub_device *ub_create(struct file *file)
{
	...
	if (!dev) {
		printk(KERN_WARNING "ublock: create: couldn't allocate ub_device\n");
		goto enomem;
	}

	dev->disk = alloc_disk(BLOCK_MINORS);
	if (!dev->disk) {
		printk(KERN_WARNING "ublock: create: alloc_disk failed\n");
		goto enomem;
	}
	...
	if (!dev->queue) {
		printk(KERN_WARNING "ublock: create: blk_init_queue_failed\n");
		goto enomem;
	}
...
static int ub_control_open(struct inode *inode, struct file *file)
{
	...
	dev = ub_create(file);
	if (!dev) {
		printk(KERN_WARNING "ublock: control open: ub_create failed\n");
		return -ENOMEM;
	}
...
static int ub_block_init(void)
{
	...
	if (ub_block_major < 0) {
		printk(KERN_ERR "ublock: register_blkdev failed\n");
		return -ENOMEM;
	}
...
static int ub_control_init(void)
{
	...
	if (result < 0) {
		printk(KERN_ERR "ublock: misc_register failed\n");
		return result;
	}
...
static int __init ub_init(void)
{
	...
	if (!ub_wq) {
		result = -ENOMEM;
		printk(KERN_ERR "ublock: create_singlethread_workqueue failed\n");
		goto fail1;
	}
	...
	printk(KERN_INFO "ublock: Userspace block device driver loaded\n");
