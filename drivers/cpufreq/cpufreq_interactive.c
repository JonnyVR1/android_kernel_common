...
static void update_load(int cpu, u64 *pnow)
{
...
static int evaluate_load(int cpu, u64 *pnow, unsigned int *ploadadjfreq)
{
	struct cpufreq_interactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	...
	spin_lock(&pcpu->load_lock);
	update_load(cpu, pnow);
	delta_time = (unsigned int) (*pnow - pcpu->cputime_speedadj_timestamp);
	...
	*ploadadjfreq = (unsigned int) cputime_speedadj * 100;
	ret = *ploadadjfreq / pcpu->target_freq;
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	cpu_load = evaluate_load(data, &now, &loadadjfreq);
