...
retry:
	...
	/* slow path: take the lock, decrement from 1 to 0, and erase node */
	write_lock(&vma_name_cache_lock);

	old = atomic_cmpxchg(&vma_name->refcount, 1, 0);
	if (old == 1)
		rb_erase(&vma_name->rb_node, &vma_name_cache);

	write_unlock(&vma_name_cache_lock);

	/* raced with a call to vma_name_get, refcount is no longer 1 */
	if (old != 1)
		goto retry;
...
static struct vma_name *vma_name_create(struct qstr *name)
{
	...
	vma_name = kzalloc(sizeof(struct vma_name) + name->len + 1, GFP_KERNEL);
	if (!vma_name)
		return NULL;
