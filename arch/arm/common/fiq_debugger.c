...
static int initial_no_sleep = true;
#else
static int initial_no_sleep;
#endif
...
static int report_trace(struct stackframe *frame, void *d)
{
	...
	if (sts->depth) {
		debug_printf(sts->state, "  pc: %p (%pF), lr %p (%pF), sp %p, fp %p\n",
					...
					struct frame_tail *tail)
{
	...
	if (__copy_from_user_inatomic(buftail, tail, sizeof(buftail))) {
		debug_printf(state, "  failed to copy frame pointer %p\n", tail);
		return NULL;
	}
		...
		struct pt_regs * const regs, unsigned int depth, void *ssp)
{
	...
	*current_thread_info() = *real_thread_info;
	...
	if (!user_mode(regs)) {
		...
		debug_printf(state, "  pc: %p (%pF), lr %p (%pF), sp %p, fp %p\n",
...
static void sleep_timer_expired(unsigned long data)
{
	...
	if (state->uart_clk_enabled && !state->no_sleep) {
		...
		enable_wakeup_irq(state->wakeup_irq);
		set_irq_wake(state->wakeup_irq, 1);
	}
...
static irqreturn_t debug_irq(int irq, void *dev)
{
	...
	if (state->tty) {
		...
		for (i = 0; i < count; i++) {
			...
			if (fiq_debugger_ringbuf_consume(state->tty_rbuf, 1) != 1)
...
static void debug_fiq(struct fiq_glue_handler *h, void *regs, void *svc_sp)
{
	...
	if (this_cpu != state->current_cpu) {
		unsigned int old_cpu = state->current_cpu;
	...
	while ((c = debug_getc(state)) != FIQ_DEBUGGER_NO_CHAR) {
		...
		if (!state->debug_enable) {
			...
			if (state->debug_count) {
				...
				state->debug_count = 0;
				debug_exec(state, state->debug_buf, regs, svc_sp);
			} else {
				debug_prompt(state);
			}
		}
...
static int fiq_debugger_tty_init(struct fiq_debugger_state *state)
{
	...
	state->tty_driver->init_termios.c_cflag = B115200 | CS8 | CREAD | HUPCL | CLOCAL;
	state->tty_driver->init_termios.c_ispeed = state->tty_driver->init_termios.c_ospeed = 115200;
...
static int fiq_debugger_probe(struct platform_device *pdev)
{
	...
	if (state->wakeup_irq >= 0) {
		...
		ret = set_irq_wake(state->wakeup_irq, 1);
		if (ret)
