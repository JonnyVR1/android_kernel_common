				...
				struct operation_decision *src)
{
	...
	if ((dest->specified & OPERATION_ALLOWED) && dest->allowed)
		*(dest->allowed) = *(src->allowed);
	if ((dest->specified & OPERATION_AUDITALLOW) && dest->auditallow)
		*(dest->auditallow) = *(src->auditallow);
	if ((dest->specified & OPERATION_AUDITDENY) && dest->auditdeny)
...
int avc_operation_add_cmd(struct operation *o, u16 cmd) {
	...
	if (avc_operation_in_range(o, cmd))
               return 1;
			...
			u16 tclass, u32 seqno, u8 operation_specified)
{
	...
	case AVC_CALLBACK_GRANT:
		...
		if (!node->ae.od || !(node->ae.od->specified & OPERATION_ALLOWED))
			break;
