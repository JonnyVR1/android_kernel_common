...
struct avc_operation_node {
	...
	struct list_head od_list; /* list of operation_decision_node */
};
...
void __init avc_init(void)
{
	...
	avc_node_cachep = kmem_cache_create("avc_node", sizeof(struct avc_node),
					     0, SLAB_PANIC, NULL);
	avc_operation_node_cachep = kmem_cache_create(
			"avc_operation_node", sizeof(struct avc_operation_node),
			0, SLAB_PANIC, NULL);
	avc_operation_decision_node_cachep = kmem_cache_create(
			"avc_operation_decision_node", sizeof(struct avc_operation_decision_node),
			0, SLAB_PANIC, NULL);
	avc_operation_perm_cachep = kmem_cache_create(
			"avc_operation_perm", sizeof(struct operation_perm),
			0, SLAB_PANIC, NULL);
					...
					struct avc_operation_node *ops_node)
{
	...
	if (!list_empty(&ops_node->od_list)) {
		list_for_each_entry(od_node, &ops_node->od_list, od_list) {
...
static inline unsigned int avc_operation_has_perm(struct operation_perm *op, u16 cmd)
{
	u8 num = cmd & 0xff;
	if (!op)
		return 0;
	return security_operation_test(op->perms, num);
}
...
static void avc_operation_allow_perm(struct avc_operation_node *node, u16 cmd)
{
	...
	if (!security_operation_test(node->ops.type, type))
...
static void avc_operation_decision_free(struct avc_operation_decision_node *od_node)
{
	struct operation_decision *od;
	od = &od_node->od;
	if (od->allowed)
					...
					struct operation_decision *src)
{
	...
	if (dest->specified & OPERATION_ALLOWED)
		memcpy(&dest->allowed->perms[0], &src->allowed->perms[0],
				sizeof(src->allowed->perms));
	if (dest->specified & OPERATION_AUDITALLOW)
		memcpy(&dest->auditallow->perms[0], &src->auditallow->perms[0],
				sizeof(src->auditallow->perms));
	if (dest->specified & OPERATION_AUDITDENY)
		memcpy(&dest->auditdeny->perms[0], &src->auditdeny->perms[0],
				sizeof(src->auditdeny->perms));
}

static struct avc_operation_decision_node *avc_operation_decision_alloc(u8 specified)
{
...
static int avc_add_operation(struct avc_node *node, struct operation_decision *od)
{
	...
	list_add(&dest_od->od_list, &node->ae.ops_node->od_list);
	return 0;
}
...
static struct avc_operation_node *avc_operation_alloc(void)
{
	...
	ops = kmem_cache_zalloc(avc_operation_node_cachep, GFP_ATOMIC|__GFP_NOMEMALLOC);
	if (!ops)
		return ops;
	INIT_LIST_HEAD(&ops->od_list);
	return ops;
}
			...
			u32 flags)
{
	...
	case AVC_CALLBACK_GRANT:
		...
		if (node->ae.ops_node && (flags & AVC_NO_CMD))
...
{
	rcu_read_unlock();
	INIT_LIST_HEAD(&ops_node->od_list);
			...
			u16 cmd, struct common_audit_data *ad)
{
	...
	if (unlikely(!od)) {
		...
		rcu_read_lock();
		avc_update_node(0, requested, cmd, ssid, tsid, tclass,
				avd.seqno, &od_local, 0);
	} else {
...
decision:
	denied = requested & ~(avd.allowed);
	if (unlikely(denied))
		rc = avc_denied(ssid, tsid, tclass, requested, cmd, AVC_NO_CMD, &avd);
