					...
					struct operation_decision *src)
{
	...
	if (dest->specified & OPERATION_AUDITDENY)
		memcpy(dest->auditdeny->perms, src->auditdeny->perms,
					...
					u32 *deniedp)
{
	...
	if (unlikely(denied)) {
		...
		if (audited && od && (od->specified & OPERATION_AUDITDENY) &&
				!avc_operation_has_perm(od->auditdeny, cmd))
			audited &= ~requested;
	} else if (result) {
		audited = denied = requested;
	} else {
		audited = requested & avd->auditallow;
		if (audited && od && (od->specified & OPERATION_AUDITALLOW) &&
				!avc_operation_has_perm(od->auditallow, cmd))
			audited &= ~requested;
	}
...
/* only copy decision information relevant to this command */
			...
			struct operation_decision *src)
{
	...
	if (dest->specified & OPERATION_AUDITDENY)
		dest->auditdeny->perms[i] = src->auditdeny->perms[i];
}
			...
			u16 cmd, struct common_audit_data *ad)
{
	...
	if (unlikely(!od)) {
		...
		rcu_read_lock();
		avc_update_node(AVC_CALLBACK_ADD_OPERATION, requested, cmd, ssid, tsid, tclass,
	...
	if (od && (od->specified & OPERATION_ALLOWED) &&
			!avc_operation_has_perm(od->allowed, cmd))
		avd.allowed &= ~requested;
