...
static void context_struct_compute_optype(
		struct operations *ops,
		...
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			for (node = avtab_search_node(&policydb.te_avtab, &avkey);
				...
				if (node->key.specified == AVTAB_ALLOWED)
					...
					context_struct_compute_optype(ops, node);
			}
...
static void od_init(struct operation_decision *od)
{
	int i;
	od->specified = 0;
	for (i = 0; i < 8; i++) {
		od->allowed->perms[i] = 0;
		od->auditallow->perms[i] = 0;
		od->auditdeny->perms[i] = 0;
	}
}

static void compute_operation(struct operation_decision *od,
					struct avtab_node *node)
{
	int i;
	struct operation_perm *op_perms;
	...
	if ((node->key.specified == AVTAB_OPTYPE_ALLOWED) ||
		...
		op_perms = od->allowed;
			...
			(node->key.specified == AVTAB_OPNUM_AUDITALLOW)) {
		od->specified |= OPERATION_AUDITALLOW;
		op_perms = od->auditallow;
	} else if ((node->key.specified == AVTAB_OPTYPE_AUDITDENY) ||
			(node->key.specified == AVTAB_OPNUM_AUDITDENY)) {
		od->specified |= OPERATION_AUDITDENY;
		op_perms = od->auditdeny;
	}
	if (node->key.specified & AVTAB_OPTYPE) {
		for (i = 0; i < 8; i++)
			op_perms->perms[i] = 0xffffffff;
	} else if (node->key.specified & AVTAB_OPNUM) {
		for (i = 0; i < 8; i++)
			op_perms->perms[i] |= node->datum.u.ops->op.perms[i];
	}
}
