					...
					struct operation *ops)
{
	...
	unsigned int i, j;
	unsigned int operation_reset = 0;
		...
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			for (node = avtab_search_node(&policydb.te_avtab, &avkey);
				...
				if (node->key.specified == AVTAB_ALLOWED)
					...
					services_compute_operation_type(ops, node, &operation_reset);
			}
					...
					struct avtab_node *node)
{
	...
	if (node->key.specified & AVTAB_OPNUM) {
		...
		if (!security_operation_test(node->datum.u.ops->op.perms, od->type))
			return;
	}
	...
	if (node->key.specified == AVTAB_OPTYPE_ALLOWED) {
		...
		memset(od->auditallow->perms, 0xff, sizeof(od->auditallow->perms));
	} else if (node->key.specified == AVTAB_OPTYPE_AUDITDENY) {
		od->specified |= OPERATION_AUDITDENY;
		memset(od->auditdeny->perms, 0, sizeof(od->auditdeny->perms));
	} else if (node->key.specified == AVTAB_OPNUM_ALLOWED) {
		od->specified |= OPERATION_ALLOWED;
		for (i = 0; i < ARRAY_SIZE(od->allowed->perms); i++)
			od->allowed->perms[i] |= node->datum.u.ops->op.perms[i];
	} else if (node->key.specified == AVTAB_OPNUM_AUDITALLOW) {
		od->specified |= OPERATION_AUDITALLOW;
		for (i = 0; i < ARRAY_SIZE(od->auditallow->perms); i++)
			od->auditallow->perms[i] |= node->datum.u.ops->op.perms[i];
	} else if (node->key.specified == AVTAB_OPNUM_AUDITDENY) {
		od->specified |= OPERATION_AUDITDENY;
		for (i = 0; i < ARRAY_SIZE(od->auditdeny->perms); i++)
			od->auditdeny->perms[i] &= node->datum.u.ops->op.perms[i];
	} else {
		BUG();
	}
}
				...
				struct operation_decision *od)
{
	...
	memset(od->auditdeny->perms, 0xff, sizeof(od->auditdeny->perms));
	...
	ebitmap_for_each_positive_bit(sattr, snode, i) {
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			cond_compute_operation(&policydb.te_cond_avtab, &avkey, od);
		}
	}
out:
	read_unlock(&policy_rwlock);
