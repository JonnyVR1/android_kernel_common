...
{
	struct operation_range tmp[MAX_OPERATION_RANGE_SIZE];
	size_t i = 0;
	size_t j = 0;
	size_t k = 0;
	/* sort into tmp */
	while (j < a->len && k < b->len) {
		...
		/* a and b overlap, merge */
			...
			if (a->range[j].low <= b->range[k].low) {
				if (b->range[k].high + 1 >= a->range[j].high) {
					a->range[j].high = b->range[k].high;
					k++;
				...
				if (a->range[j].high > b->range[k].high) {
					b->range[k].high = a->range[j].high;
					j++;
	...
	while (j < a->len && i < MAX_OPERATION_RANGE_SIZE) {
		tmp[i] = a->range[j];
		i++;
		j++;
	}
	while (k < b->len && i < MAX_OPERATION_RANGE_SIZE) {
		tmp[i] = b->range[k];
		i++;
		k++;
	}
