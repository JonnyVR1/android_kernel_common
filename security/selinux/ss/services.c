					...
					struct avtab_node *node)
{
	...
	if (node->key.specified & AVTAB_OPNUM) {
		if (!security_operation_test(node->datum.u.ops->perms, type))
			return;
	}
	...
	if (node->key.specified & AVTAB_OPTYPE) {
		memset(op_perms->perms, 0xff, sizeof(op_perms->perms));
	} else if (node->key.specified & AVTAB_OPNUM) {
		for (i = 0; i < ARRAY_SIZE(op_perms->perms); i++)
