...
int avtab_write_item(struct policydb *p, struct avtab_node *cur, void *fp)
{
	...
	if (vers < POLICYDB_VERSION_IOCTL_OPERATIONS ||
		...
		buf16[0] = cur->datum.u.ops->len;
		rc = next_entry(buf16, fp, sizeof(u16));
		if (rc) {
			printk(KERN_ERR "SELinux: avtab: truncated entry\n");
			return rc;
		}
		for (i = 0; i < cur->datum.u.ops->len; i++) {
			buf16[0] = cpu_to_le16(cur->datum.u.ops->range[i].low);
			buf16[1] = cpu_to_le16(cur->datum.u.ops->range[i].high);
			rc = put_entry(buf16, sizeof(u16), 2, fp);
			if (rc) {
				printk(KERN_ERR "SELinux: avtab: truncated entry\n");
				return rc;
			}
