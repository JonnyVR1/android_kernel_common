...
#define SUSPEND_BINS 32
static atomic_t time_in_suspend_bins[SUSPEND_BINS];
#endif
		...
		unsigned long pm_event, void *unused)
{
	...
	case PM_POST_SUSPEND:
		...
		atomic_inc(time_in_suspend_bins + fls(suspend_time.tv_sec));
...
static int suspend_time_debug_show(struct seq_file *s, void *data)
{
	...
	seq_printf(s, "------------------\n");
	for (bin = 0; bin < SUSPEND_BINS; bin++) {
		if (atomic_read(time_in_suspend_bins + bin) == 0)
			continue;
		seq_printf(s, "%4d - %4d %4u\n",
			bin ? 1 << (bin - 1) : 0, 1 << bin,
				atomic_read(time_in_suspend_bins + bin));
	}
	return 0;
}
