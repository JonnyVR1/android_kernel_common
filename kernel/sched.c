...
/* alloc_percpu macro uses typeof so we must define a type here
 *
 * TODO: It would be nice to be able use the cpufreq table size defined in arch/board
 * files.
 */
#define CPUFREQ_USAGE_MAX 32
typedef struct { u64 usage[CPUFREQ_USAGE_MAX]; } cpufreq_usage_t;
#endif
...
static void cpuacct_charge(struct task_struct *tsk, u64 cputime)
{
	...
	for (; ca; ca = ca->parent) {
		...
		if (cpufreq_index > CPUFREQ_USAGE_MAX)
			WARN_ONCE(1,"cpuacct_charge: cpufreq_index: %d exceeds"
					" max table size\n", cpufreq_index);
		else
