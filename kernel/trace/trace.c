...
static unsigned saved_cmdline_num = 256;
...
static int trace_init_cmdlines(void)
{
	...
	map_cmdline_to_pid = kmalloc(saved_cmdline_num * sizeof(unsigned), GFP_KERNEL);

	if (!map_cmdline_to_pid)
		return 1;


	saved_tgids = kzalloc(saved_cmdline_num * sizeof(unsigned), GFP_KERNEL);

	if (!saved_tgids)
		goto free_cmd_2_pid;

	/* setup the 2d array */
	len = (saved_cmdline_num * TASK_COMM_LEN);
	temp = kzalloc(len, GFP_KERNEL);

	if (!temp)
		goto free_savedtgids;

	saved_cmdlines = kmalloc(saved_cmdline_num * sizeof(char *), GFP_KERNEL);

	if (!saved_cmdlines)
		goto free_tmp;

	for (i = 0; i < saved_cmdline_num; i++)
		saved_cmdlines[i] = &(temp[TASK_COMM_LEN * i]);

	memset(&map_pid_to_cmdline, NO_CMDLINE_MAP, sizeof(map_pid_to_cmdline));
	memset(map_cmdline_to_pid, NO_CMDLINE_MAP, saved_cmdline_num * sizeof(unsigned));
	cmdline_idx = 0;
...
static int extend_mappings(void)
{
	...
	unsigned i;
	size_t newsize = saved_cmdline_num + 256;
	...
	map_cmdline_to_pid = kmalloc(newsize * sizeof(unsigned), GFP_ATOMIC);

	if (unlikely(!map_cmdline_to_pid))
		goto failed_mapcmdline;

	saved_tgids = kzalloc(newsize * sizeof(unsigned), GFP_ATOMIC);

	if (unlikely(!saved_tgids))
	...
	saved_cmdlines = kmalloc(newsize * sizeof(char*), GFP_ATOMIC);

	if (unlikely(!saved_cmdlines))
		goto failed_cmdlines;


	/* all allocs completed, move data and free previous allocations */

	memcpy(map_cmdline_to_pid, tmpcmdline2pid, saved_cmdline_num * sizeof(unsigned));
	memcpy(saved_tgids, tmpsavedtgids, saved_cmdline_num * sizeof(unsigned));
	memcpy(newdata, tmpcmdlines_data, saved_cmdline_num * TASK_COMM_LEN);

	/* init the back size of the array*/
	memset((map_cmdline_to_pid + saved_cmdline_num),
	       NO_CMDLINE_MAP, 256 * sizeof(unsigned));
...
static void trace_save_cmdline(struct task_struct *tsk)
{
	...
	retry:
		idx = (cmdline_idx + 1) % saved_cmdline_num;
				...
				size_t cnt, loff_t *ppos)
{
	...
	file_buf = kmalloc(saved_cmdline_num*(16+TASK_COMM_LEN), GFP_KERNEL);
	if (!file_buf)
		return -ENOMEM;
				...
				size_t cnt, loff_t *ppos)
{
	...
	file_buf = kmalloc(saved_cmdline_num*(16+1+16), GFP_KERNEL);
	if (!file_buf)
		return -ENOMEM;
