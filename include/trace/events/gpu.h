 ...
 *
 * The prev_ctx_id argument identifies the previous context that was running on
 * the GPU hardware block.  A value of 0 indicates that the hardware block was
 * previously idle.
 *
 * The prev_prio argument indicates the priority of the previous context at the
 ...
 * higher priority.
 *
 * The next_work_id argument identifies the batch of work that the GPU will be
 * working on.  This should correspond to a work_id that was previously traced
 * as a gpu_work_queue event when the batch of work was sent to the GPU.
 */
TRACE_EVENT(gpu_sched_switch,

	TP_PROTO(const char *gpu_name, u64 timestamp,
		u32 prev_ctx_id, s32 prev_prio,
		u32 next_ctx_id, s32 next_prio, u32 next_work_id),

	TP_ARGS(gpu_name, timestamp, prev_ctx_id, prev_prio, next_ctx_id, next_prio,
		next_work_id),

	TP_STRUCT__entry(
		__string(       gpu_name,       gpu_name        )
		__field(        u64,            timestamp       )
		__field(        u32,            prev_ctx_id     )
		__field(        s32,            prev_prio       )
		__field(        u32,            next_ctx_id     )
		__field(        s32,            next_prio       )
		__field(        u32,            next_work_id    )
	),

	TP_fast_assign(
		__assign_str(gpu_name, gpu_name);
		__entry->timestamp = timestamp;
		__entry->prev_ctx_id = prev_ctx_id;
		__entry->prev_prio = prev_prio;
		__entry->next_ctx_id = next_ctx_id;
		__entry->next_prio = next_prio;
		__entry->next_work_id = next_work_id;
	),

	TP_printk("gpu_name=%s ts=%5llu.%06lu prev_ctx_id=%lu prev_prio=%ld next_ctx_id=%lu next_prio=%ld next_work_id=%lu",
		__get_str(gpu_name),
		(unsigned long long)show_secs_from_ns(__entry->timestamp),
		(unsigned long)show_usecs_from_ns(__entry->timestamp),
		(unsigned long)__entry->prev_ctx_id,
		(long)__entry->prev_prio,
		(unsigned long)__entry->next_ctx_id,
		(long)__entry->next_prio,
		(unsigned long)__entry->next_work_id)
);

/*
 * The gpu_work_queue event indicates that a batch of work has been queued up
 * to be processed by the GPU.  This event is not intended to indicate that the
 * batch of work has been submitted to the GPU hardware, but rather that it has
 * been submitted to the GPU kernel driver.
 *
 ...
 *
 * The ctx_id field identifies the GPU context in which the batch of work being
 * queued is to be run.
 *
 * The work_id field identifies the batch of work being queued within the given
 * GPU context.  The first batch of work submitted for a given GPU context
 * should have a work_id of 0, and each subsequent batch of work should
 * increment the work_id by 1.
 *
 * The type field identifies the type of the work being queued.  The types of
 * work may be different for different GPU hardware.  For example, a GPU may
 * differentiate between "2D", "3D", and "compute" work batches.
 */
TRACE_EVENT(gpu_work_queue,

	TP_PROTO(u32 ctx_id, u32 work_id, const char *type),

	TP_ARGS(ctx_id, work_id, type),
	...
	TP_fast_assign(
		...
		__entry->work_id = work_id;
		__assign_str(type, type);
	),

	TP_printk("ctx_id=%lu work_id=%lu type=%s",
		(unsigned long)__entry->ctx_id,
		(unsigned long)__entry->work_id,
		__get_str(type))
);
