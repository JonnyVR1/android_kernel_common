...
static int __init verify_verity_signature(void)
{
	...
	pks = table_make_digest(PKEY_HASH_SHA256,
			(void *)verity_metadata.verity_table,
...
static int __init verity_create_dev(char *dev_name)
{
	...
	if (DEV == 0) {
		pr_init_info("Waiting for device %s...\n",
			dev_name);
	...
	pr_init_info("dev_t %u %u\n", MAJOR(DEV), MINOR(DEV));
	return create_dev(dev_name, DEV);
}

static int __init get_metadata_offset(uint64_t *device_size)
{
	...
	*device_size = i_size_read(bdev->bd_inode) - VERITY_METADATA_SIZE;
	bdput(bdev);
...
static int __init handle_error(int mode)
{
	if (mode == VERITY_MODE_RESTART) {
		...
		pr_init_err("Mounting root with verity disabed");
	}
...
{
	int ret = 0;
	char *arg, *verity_table_args[10], *table_ptr;
	int i = 0, arg_len;
	...
	if (ret == VERITY_STATE_DISABLE) {
		pr_init_err("Verity disabled! Mounting root with verity disabed");
		goto err;
	}
...
struct dm_setup_verity * __init verity_run_setup(char *rootdev)
{
	int mode = verity_mode();
